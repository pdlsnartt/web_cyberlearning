...
import { EsportsManager } from "../components/EsportsManager";
...
export function LabDetailPage() {
  ...
  const isManagerLab = lab.id === "esports-manager-2026";

  return (
    <article className="detailPage">
      <div className="detailHeader compactHeader">
        <span className="eyebrow">
          Lab {lab.order} / {lab.difficulty[language]}
        </span>
      ...
      <nav className="detailNav">
        <Link className="ghostButton" to="/labs">
          {language === "ru" ? "Все лабораторные" : "All labs"}
        </Link>
        <Link className="primaryButton" to="/testing">
          {language === "ru" ? "Закрепить тестом" : "Practice with test"}
        </Link>
      </nav>
    </article>
  );
